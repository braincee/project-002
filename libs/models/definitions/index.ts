...
const Content = sequelize.define<Content>('Content', {
  id: {
    primaryKey: true,
    type: DataTypes.UUID,
    defaultValue: DataTypes.UUIDV4,
...
const ContentAddress = sequelize.define<ContentAddress>('ContentAddress', {
  content_id: {
    type: DataTypes.UUID,
    allowNull: false,
    references: {
...
Address.belongsToMany(Content, { through: ContentAddress, foreignKey: 'address_id'});

Content.belongsToMany(Address, { through: ContentAddress, foreignKey: 'content_id'});
